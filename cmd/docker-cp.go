...
func cpDockerContainer(tarformers *executor.TarFormers,
	cid, srcPath, dir, file, spec, specOut string) error {
	...
	cmds := []string{
		"/bin/bash", "-c",
		"docker cp " + fmt.Sprintf("%s:%s", cid, srcPath) + " -",
	}
...
func newDockerCpCommand(config *specs.Config) *cobra.Command {
	var cmd = &cobra.Command{
		Use:   "docker-cp [container-id] [src_path]",
		Short: "Copy files from a docker container path to a specified directory or to a file.",
		Long: `Copy files from a  docker container to a specified directory:

$> tar-formers docker-cp <container-id> <src-path> --todir ./out

Copy files from a docker container, apply filter and generate a new tarball.

$> tar-formers docker-cp <container-id> <src-path> --to /mycontainer.tar.gz --specs spec.yml

Copy files from a docker container, apply filter on both reader and writer
...
`,
		...
		PreRun: func(cmd *cobra.Command, args []string) {
			if len(args) < 2 {
				fmt.Println("No container id or src-path present.")
				os.Exit(1)
			}
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			err := cpDockerContainer(
				tarformers, args[0], args[1], todir,
				to, specfile, out)
